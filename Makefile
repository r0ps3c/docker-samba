...
clean-test:
	docker rm -f samba-test-* 2>/dev/null || true
	docker volume rm -f samba-test-* 2>/dev/null || true
	docker network rm samba-test-* 2>/dev/null || true

version: build
	@docker run --rm --entrypoint sh $(PKGNAME):$(TAG) -c 'apk info samba 2>/dev/null | grep "^samba-" | head -1 | cut -d- -f2 | cut -dr -f1'

show-version: build
	@FULL_VER=$$(docker run --rm --entrypoint sh $(PKGNAME):$(TAG) -c 'apk info samba 2>/dev/null | grep "^samba-" | head -1 | cut -d- -f2 | cut -dr -f1'); \
